...
export default function ResourcePage() {
  ...
  useEffect(() => {
    ...
    const foundResource = Object.values(resources).find((r: any) => r.id === id) as Resource | undefined
        ...
        <CardContent>
          ...
          {resource.fileUrl ? (
            ...
            <p className="mb-4 text-yellow-600">
              The full content for this resource is not available yet. Please check back later.
            </p>
          )}
          <p className="mb-4 font-bold">{resource.isPaid ? `Price: $${resource.price.toFixed(2)}` : "Free"}</p>
          <Button onClick={handleDownload}>{resource.isPaid ? "Purchase and Download" : "Download Now"}</Button>
        </CardContent>
      </Card>
    </div>
  )
